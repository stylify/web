export default {
	...
	'Production Build and Selectors Mangling': '生产构建与选择器混用',
	'When you execute {{code}} during a development, selector are going to be generated in the same way as they are written in the class attributes.': '当你在开发过程中执行{{code}}时，选择器的生成方式与它们在类属性中的写法相同。',
	'In production build, that is executed by {{code}}, selectors are minified from long <code>color:blue</code> to short <code>a</code>.': '在生产构建中，即由{{code}}执行时，选择器会从长的<code>color:blue</code>减为短的<code>a</code>。',
	'Selectors are rewritten directly within files/templates. This is because Stylify matches selectors only in selected areas (to prevent unwanted characters to be matched) like <code>class</code>, <code>className</code>, <code>:class</code>. However, frameworks compiles these attributes like <code>class="color:blue"</code> to something like <code>add_attribute(button, "class", "color:blue")</code> under the hood.': '选择符在文件/模板中被直接重写。这是因为Stylify只在选定的区域匹配选择器（以防止不需要的字符被匹配），如<code>class</code>, <code>className</code>, <code>:class</code>。然而，框架将这些属性如<code>class="color:blue"</code>编译为类似<code>add_attribute(button, "class", "color:blue")</code>的东西。',
	'This can cause, that some original selectors will not be rewritten because they are not matched. Stylify could have matching areas defined for frameworks\' compiled output, but this could break with any release. Therefore is safer to rewrite them directly within templates during a production build, where it takes no effect on development environment because this command is mostly executed in build pipeline in Github/Gitlab or during a Vercel/Netlify build and deploy.': '这可能会导致，一些原始的选择器不会被重写，因为它们没有被匹配。Stylify可以为框架的编译输出定义匹配区域，但这可能会在任何版本中被破坏。因此，在生产构建过程中直接在模板中重写它们是比较安全的，这对开发环境没有影响，因为这个命令主要是在Github/Gitlab的构建管道中或在Vercel/Netlify构建和部署时执行。',
	'How to disable mangling and selectors rewritting': '如何禁用混搭和选择器重写',
	'If you want to disable mangling, for example for testing production build locally or even in production build, just add the following into the config:': '如果你想禁用混搭，例如在本地测试生产构建或甚至在生产构建中，只需在配置中添加以下内容：',
	'Sometime, the class attribute can be consistent during the build within the bundler (it\'s random). You may try to disable rewriting selectors within files like this': '有时，类属性在bundler内的构建过程中可能是一致的（它是随机的）。你可以尝试在文件内禁止重写选择器，比如这样',
	'If selectors are rewritten correctly even after this configuration, you can keep it that way. Otherwise, if you still want the rewriting to be disabled, you will have to dig into Stylify Compiler <code>rewriteSelectors</code> method and see what comes as an input into this method and configure a correct selectorsArea, so Stylify can process it correctly.': '如果选择器在这样的配置后也能正确重写，你可以保持这种方式。否则，如果你仍然想禁止重写，你就必须深入研究Stylify编译器的<code>rewriteSelectors</code>方法，看看有什么东西作为输入到这个方法，并配置一个正确的selectorsArea，这样Stylify就可以正确地处理它。',
	'Selectors mangling in files cannot be disabled in frameworks that are not based on Javascript (PHP, C#, Java), because their templates are not compiled by Vite/Webpack/Rollup/ESbuild.': '在非基于Javascript的框架（PHP、C#、Java）中，文件中的选择器处理不能被禁用，因为他们的模板没有被Vite/Webpack/Rollup/ESbuild编译。',
	'Editor': '编辑',
	'Dev CSS': '开发CSS',
