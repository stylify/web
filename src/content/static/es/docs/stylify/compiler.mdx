...
const compilerConfig = {
	macros: {
		'color:(\\S+?)': ({macroMatch, selectorProperties}) => {
			...
			// También puedes usar addMultiple({})
...
const compilerConfig = {
	...
	// Por defecto, las variables se sustituyen por su valor. Si se activa esta opción, se
	// son reemplazadas por var(--variableName) y pueden ser más dinámicas.
	// Esto es genial por ejemplo para cambiar entre modo claro y oscuro.
...
Por defecto Stylify CSS soporta algunas sintaxis de Vue, React, Lit, AlpineJS y Nette. En caso de que alguno de los atributos de clase no coincida, añade la opción selectorsAreas con una expresión regular para que coincida.
...
Algunas opciones de configuración se pueden definir directamente en un fichero usando `content options`. Es bueno guardar la definición de por ejemplo un componente con su HTML.
