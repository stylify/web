...
### Variables CSS
Cuando quieras activar las variables CSS, simplemente añade `replaceVariablesByCssVariables` en la configuración de stylify. Esto reemplazará la variable por una variable CSS en lugar de su valor.

<Note>Las variables CSS estarán activadas por defecto en la próxima versión 0.6.</Note>

Stylify intenta ser estricto por defecto. Cuando las variables CSS están habilitadas, es posible que desee establecer [variables externas](/es/docs/stylify/compiler#externalvariables), para informar a Stylify, que algunas variables existen pero son externas. Si no quieres configurarlas todas, puedes desactivar la advertencia completamente.
