...
<Note>For some unknown reason, the CSS is not reloaded when an error is triggered within the Nuxt (syntax, duplicated attributes). If that happens, just hit save within the Nuxt config file or restart the dev server.</Note>
...
- Nuxt v3: [@stylify/nuxt](/en/docs/nuxt)
- Nuxt v2: [@stylify/nuxt-module](/en/docs/nuxt-module)
