...
Below is a list of a few pros and cons you will probably encounter, when working with Stylify CSS or Tailwind CSS. I have explained them more in a detail in [why Stylify post](/en/docs/get-started/why-stylify-css).
...
In Stylify, this can be done in various ways using [custom selectors](/en/docs/stylify/compiler#customselectors).
...
In [Stylify CSS](/) you can define these selectors within the config file or in a template where they are used.
...
The config from template can also be moved into the [config file](/en/docs/stylify/compiler#customselectors):
```javascript
...
In the case of Stylify, there is nothing like a "theme". There are only variables that can be defined in two ways. If variable is used globally, put it in the [global config](/en/docs/stylify/compiler#variables):
```javascript
const compilerConfig = {
	variables: {
		textColor: '#000',
...
If it is used locally, configure it within a comment in a file where it is used using [content options](/en/docs/stylify/compiler#contentoptionsprocessors). It expects javascript object without surrounding brackets:
...
## Splitting CSS
[Stylify CSS](/) allows you to split CSS bundles basically for each file separately.
