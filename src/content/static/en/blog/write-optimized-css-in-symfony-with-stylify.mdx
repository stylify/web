...
Sometime, some components are used only in one place. It doesn't make sense to define them in the `webpack.config.js`. We can also define components, variables and etc directly in the file where they are used. Stylify CSS has [content options](/en/docs/stylify/compiler#contentoptionsprocessors) for that.
...
The example can be also found in the [docs](/en/docs/integrations/symfony).
