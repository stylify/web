...
### CSS变量
当你想打开CSS变量时，只需在stylify配置中添加`replaceVariablesByCssVariables`。这将用一个CSS变量来代替变量的值。

<Note>CSS变量将在即将到来的0.6版本中被默认启用。</Note>

Stylify在默认情况下会努力做到严格。当CSS变量被启用时，你可能想设置[external variables](/zh/docs/stylify/compiler#externalvariables)，以通知Stylify，有些变量是存在的，但却是外部的。如果你不想配置所有的变量，你可以完全停用这个警告。
